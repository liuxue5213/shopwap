 ...
 * @return string
 */
function xml_encode($data, $root = 'swtouch', $item = 'item', $attr = '', $id = 'id', $encoding = 'utf-8') {
    if (is_array($attr)) {
        $_attr = array();
...
function cookie($name = '', $value = '', $option = null) {
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            // 设置cookie
            if (is_array($value)) {
                $value = 'swtouch:' . json_encode(array_map('urlencode', $value));
            }
 ...
 * @param string $appid
 */
function site_info($appid = SWTOUCH_AUTH_KEY){
