...
class UserController extends CommonController {
    ...
    public function order_detail() {
        ...
        if ($order['order_status'] == OS_SPLITED) {
            ...
            if ($order['shipping_status'] == SS_SHIPPED) {
                ...
                @$order['handler'] = '<a class="btn btn-info ect-colorf" type="button" href="javascript:;">' . L('ss_received') . '</a>';
            } else {
                if ($order['pay_status'] == PS_UNPAYED) {
                    ...
                    // @$order['handler'] = "<a class=\"btn btn-info ect-colorf\" href=\"javascript:;\">" . L('view_order') . "</a>";
                }
            }
        } else {
            $order['handler'] = '<a class="btn btn-info ect-colorf" type="button" href="javascript:;">' . L('os.' . $order['order_status']) . '</a>';
        }
        if ($order === false) {
            SWTouch::err()->show(L('back_home_lnk'), './');
            exit();
        }
    ...
    public function affirm_received() {
        ...
        if (model('Users')->affirm_received($order_id, $this->user_id)) {
            ...
            SWTouch::err()->show(L('order_list_lnk'), url('order_list'));
        }
    }
    ...
    public function edit_surplus() {
        ...
        if ($surplus <= 0) {
            SWTouch::err()->add(L('error_surplus_invalid'));
            SWTouch::err()->show(L('order_detail'), url('order_detail', array(
                'order_id' => $order_id
            )));
        }
        ...
        if ($order['pay_status'] != PS_UNPAYED || $order['order_amount'] <= 0) {
            SWTouch::err()->add(L('error_order_is_paid'));
            SWTouch::err()->show(L('order_detail'), url('order_detail', array(
                'order_id' => $order_id
            )));
        }
        ...
        if ($surplus > $user['user_money'] + $user['credit_line']) {
            SWTouch::err()->add(L('error_surplus_not_enough'));
            SWTouch::err()->show(L('order_detail'), url('order_detail', array(
                'order_id' => $order_id
            )));
        }
    ...
    public function bonus() {
        if (IS_POST) {
            ...
            if (model('Users')->add_bonus($this->user_id, $bonus_sn)) {
                ...
                SWTouch::err()->show(L('back_up_page'), url('bonus'));
            }
        }
        // 分页
        $filter['page'] = '{page}';
    ...
    public function add_booking() {
        if (IS_POST) {
            ...
            if (model('ClipsBase')->add_booking($booking)) {
                ...
                SWTouch::err()->show(L('booking_list_lnk'), url('booking_list'));
            }
        }
        $goods_id = I('get.id', 0);
        if ($goods_id == 0) {
    ...
    public function login() {
        ...
        if (IS_POST) {
            ...
            if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2))) {
                ...
                // 检查验证码
                if ($_SESSION['swtouch_verify'] !== strtoupper($_POST['captcha'])) {
                    show_message(L('invalid_captcha'), L('relogin_lnk'), url('login', array(
                        'referer' => urlencode($this->back_act)
                            )), 'error');
                }
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            elseif (1 == $enabled_sms) {
                ...
                SWTouch::err()->show(L('sign_up'), url('register'));
            }
