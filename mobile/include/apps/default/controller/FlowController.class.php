...
class FlowController extends CommonController {
    ...
    public function add_to_cart() {
        ...
        if (!is_numeric($goods->number) || intval($goods->number) <= 0) {
            ...
            if (model('Order')->addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent)) {
                ...
                $result ['message'] = SWTouch::err()->last_message();
                $result ['error'] = SWTouch::err()->error_no;
        ...
        if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
            ...
            if ($act == 'signin') {
                ...
                if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION ['login_fail'] > 2)) && gd_version() > 0) {
                    ...
                    if ($_SESSION ['swtouch_verify'] !== $_POST ['captcha']) {
                        show_message(L('invalid_captcha'));
                    }
                }
    ...
    public function select_shipping() {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
            ...
            $result ['content'] = SWTouch::$view->fetch('library/order_total.lbi');
            $this->assign('content_list', '1');
            $result ['content_list'] = SWTouch::$view->fetch('library/order_total02.lbi');
        }
    ...
    public function change_needinv() {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
            ...
            die(SWTouch::$view->fetch('library/order_total.lbi'));
            $this->assign('content_list', '1');
            $result ['content_list'] = SWTouch::$view->fetch('library/order_total02.lbi');
        }
    }
