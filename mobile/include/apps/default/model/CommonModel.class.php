    ...
    function assign_dynamic($tmp) {
        ...
        foreach ($res AS $row) {
            switch ($row['type']) {
                case 1:
                    /* 分类下的商品 */
                    SWTouch::view()->assign('goods_cat_' . $row['id'], model('Goods')->assign_cat_goods($row['id'], $row['number']));
                    break;
                case 2:
                    /* 品牌的商品 */
                    $brand_goods = model('Goods')->assign_brand_goods($row['id'], $row['number']);

                    SWTouch::view()->assign('brand_goods_' . $row['id'], $brand_goods['goods']);
                    SWTouch::view()->assign('goods_brand_' . $row['id'], $brand_goods['brand']);
                    break;
                case 3:
                    /* 文章列表 */
                    $cat_articles = model('Article')->assign_articles($row['id'], $row['number']);

                    SWTouch::view()->assign('articles_cat_' . $row['id'], $cat_articles['cat']);
                    SWTouch::view()->assign('articles_' . $row['id'], $cat_articles['arr']);
                    break;
            }
        }
    }
