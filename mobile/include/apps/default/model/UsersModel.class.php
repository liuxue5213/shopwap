...
class UsersModel extends BaseModel {
    ...
    function register($username, $password, $email, $other = array()) {
        ...
        if (!empty($shop_reg_closed)) {
            SWTouch::err()->add(L('shop_register_closed'));
        }
        /* 检查username */
        if (empty($username)) {
            SWTouch::err()->add(L('username_empty'));
        } else {
            if (preg_match('/\'\/^\\s*$|^c:\\\\con\\\\con$|[%,\\*\\"\\s\\t\\<\\>\\&\'\\\\]/', $username)) {
                SWTouch::err()->add(sprintf(L('username_invalid'), htmlspecialchars($username)));
            }
        }

        /* 检查email */
        if (empty($email)) {
            SWTouch::err()->add(L('email_empty'));
        } else {
            if (!is_email($email)) {
                SWTouch::err()->add(sprintf(L('email_invalid'), htmlspecialchars($email)));
            }
        }

        if (SWTouch::err()->error_no > 0) {
            return false;
        }

        /* 检查是否和管理员重名 */
        if (model('Users')->admin_registered($username)) {
            SWTouch::err()->add(sprintf(L('username_exist'), $username));
            return false;
        }

        if (!SWTouch::user()->add_user($username, $password, $email)) {
            if (SWTouch::user()->error == ERR_INVALID_USERNAME) {
                SWTouch::err()->add(sprintf(L('username_invalid'), $username));
            } elseif (SWTouch::user()->error == ERR_USERNAME_NOT_ALLOW) {
                SWTouch::err()->add(sprintf(L('username_not_allow'), $username));
            } elseif (SWTouch::user()->error == ERR_USERNAME_EXISTS) {
                SWTouch::err()->add(sprintf(L('username_exist'), $username));
            } elseif (SWTouch::user()->error == ERR_INVALID_EMAIL) {
                SWTouch::err()->add(sprintf(L('email_invalid'), $email));
            } elseif (SWTouch::user()->error == ERR_EMAIL_NOT_ALLOW) {
                SWTouch::err()->add(sprintf(L('email_not_allow'), $email));
            } elseif (SWTouch::user()->error == ERR_EMAIL_EXISTS) {
                SWTouch::err()->add(sprintf(L('email_exist'), $email));
            } else {
                SWTouch::err()->add('UNKNOWN ERROR!');
            }
            ...
            /* 设置成登录状态 */
            SWTouch::user()->set_session($username);
            SWTouch::user()->set_cookie($username);
    ...
    function send_regiter_hash($user_id) {
        ...
        $validate_email = __HOST__ . url('user/validate_email', array('hash' => $hash)); //SWTouch::ecs()->url() . 'user.php?act=validate_email&hash=' . $hash;

        $sql = "SELECT user_name, email FROM " . $this->pre . "users WHERE user_id = '$user_id'";
        $row = $this->row($sql);

        SWTouch::view()->assign('user_name', $row['user_name']);
        SWTouch::view()->assign('validate_email', $validate_email);
        SWTouch::view()->assign('shop_name', C('shop_name'));
        SWTouch::view()->assign('send_date', date(C('date_format')));

        $content = SWTouch::view()->fetch('str:' . $template['template_content']);
    ...
    /**
     * 判断超级管理员帐号是否存在
     * @param   string      $adminname 超级管理员帐号
     * @return  boolean
     */
    ...
    function edit_profile($profile) {
        ...
        if (!empty($profile['email'])) {
            if (!is_email($profile['email'])) {
                SWTouch::err()->add(sprintf(L('email_invalid'), $profile['email']));
        ...
        if (!SWTouch::user()->edit_user($cfg)) {
            if (SWTouch::user()->error == ERR_EMAIL_EXISTS) {
                SWTouch::err()->add(sprintf(L('email_exist'), $profile['email']));
            } else {
                SWTouch::err()->add('DB ERROR!');
            }
    ...
    function get_profile($user_id) {
        ...
        $row = SWTouch::user()->get_profile_by_name($infos['user_name']); //获取用户帐号信息
        ...
        if ($row) {
            if ($row['user_id'] == 0) {
                ...
                if ($now > $bonus_time['use_end_date']) {
                    SWTouch::err()->add(L('bonus_use_expire'));
                    return false;
                }
                ...
                if ($row['user_id'] == $user_id) {
                    //红包已经添加过了。
                    SWTouch::err()->add(L('bonus_is_used'));
                } else {
                    //红包被其他人使用过了。
                    SWTouch::err()->add(L('bonus_is_used_by_other'));
                }
            ...
            //红包不存在
            SWTouch::err()->add(L('bonus_not_exist'));
            return false;
        }
    }
    ...
    function cancel_order($order_id, $user_id = 0) {
        ...
        if (empty($order)) {
            SWTouch::err()->add(L('order_exist'));
            return false;
        }

        // 如果用户ID大于0，检查订单是否属于该用户
        if ($user_id > 0 && $order['user_id'] != $user_id) {
            SWTouch::err()->add(L('no_priv'));
        ...
        if ($order['order_status'] != OS_UNCONFIRMED && $order['order_status'] != OS_CONFIRMED) {
            SWTouch::err()->add(L('current_os_not_unconfirmed'));
        ...
        if ($order['order_status'] == OS_CONFIRMED) {
            SWTouch::err()->add(L('current_os_already_confirmed'));
        ...
        if ($order['shipping_status'] != SS_UNSHIPPED) {
            SWTouch::err()->add(L('current_ss_not_cancel'));
        ...
        if ($order['pay_status'] != PS_UNPAYED) {
            SWTouch::err()->add(L('current_ps_not_cancel'));
    ...
    function affirm_received($order_id, $user_id = 0) {
        ...
        /* 检查订单 */ elseif ($order['shipping_status'] == SS_RECEIVED) {
            SWTouch::err()->add(L('order_already_received'));

            return false;
        } elseif ($order['shipping_status'] != SS_SHIPPED) {
            SWTouch::err()->add(L('order_invalid'));
    ...
    function get_order_detail($order_id, $user_id = 0) {
        ...
        if ($order_id <= 0) {
            SWTouch::err()->add(L('invalid_order_id'));
        ...
        if ($order['pay_status'] != PS_UNPAYED) {
            ...
            SWTouch::view()->assign('virtual_card', $var_card);
        }
    ...
    function merge_user_order($from_order, $to_order, $user_id = 0) {
        if ($user_id > 0) {
            ...
            if (strlen($to_order) > 0) {
                ...
                SWTouch::err()->add(L('order_sn_empty'));
                return false;
            }
        }
    ...
    function save_order_address($address, $user_id) {
        SWTouch::err()->clean();
        /* 数据验证 */
        empty($address['consignee']) and SWTouch::err()->add(L('consigness_empty'));
        empty($address['address']) and SWTouch::err()->add(L('address_empty'));
        $address['order_id'] == 0 and SWTouch::err()->add(L('order_id_empty'));
        if (empty($address['email'])) {
            SWTouch::err()->add($GLOBALS['email_empty']);
        } else {
            if (!is_email($address['email'])) {
                SWTouch::err()->add(sprintf(L('email_invalid'), $address['email']));
            }
        }
        if (SWTouch::err()->error_no > 0) {
            return false;
        ...
        if ($row) {
            ...
            if ($row['order_status'] != OS_UNCONFIRMED) {
                SWTouch::err()->add(L('require_unconfirmed'));
                return false;
            }
        ...
        if ($feed_type == BUY_GOODS) {
            ...
            foreach ($order_res as $goods_data) {
                if (!empty($goods_data['goods_thumb'])) {
                    $url = __URL__ . $goods_data['goods_thumb']; //SWTouch::ecs()->url() . $goods_data['goods_thumb'];
                } else {
                    $url = __URL__ . C('no_picture'); //SWTouch::ecs()->url() . C('no_picture');
                }
                $link = __HOST__ . url('goods/index', array('id' => $goods_data["goods_id"])); //SWTouch::ecs()->url() . "goods.php?id=" . $goods_data["goods_id"];
    ...
    function get_affiliate() {
        if (!empty($_COOKIE['shopwapp_affiliate_uid'])) {
            $uid = intval($_COOKIE['shopwapp_affiliate_uid']);
            if ($this->row('SELECT user_id FROM ' . $this->pre . "users WHERE user_id = '$uid'")) {
                return $uid;
            } else {
                setcookie('shopwapp_affiliate_uid', '', 1);
            }
        }
