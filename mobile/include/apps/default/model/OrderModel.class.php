...
class OrderModel extends BaseModel {
    ...
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0) {
        ...
        if (empty($goods)) {
            SWTouch::err()->add(L('goods_not_exists'), ERR_NOT_EXISTS);
        ...
        if ($parent > 0) {
            ...
            $res = $this->row($sql);
            if ($res['count'] == 0) {
                SWTouch::err()->add(L('no_basic_goods'), ERR_NO_BASIC_GOODS);
        ...
        if ($goods['is_on_sale'] == 0) {
            SWTouch::err()->add(L('not_on_sale'), ERR_NOT_ON_SALE);
        ...
        if (empty($parent) && $goods['is_alone_sale'] == 0) {
            SWTouch::err()->add(L('cannt_alone_sale'), ERR_CANNT_ALONE_SALE);
        ...
        if (C('use_storage') == 1) {
            ...
            if ($num > $goods['goods_number']) {
                SWTouch::err()->add(sprintf(L('shortage'), $goods['goods_number']), ERR_OUT_OF_STOCK);
            ...
            if (model('GoodsBase')->is_spec($spec) && !empty($prod)) {
                if (!empty($spec)) {
                    ...
                    if ($num > $product_info['product_number']) {
                        SWTouch::err()->add(sprintf(L('shortage'), $product_info['product_number']), ERR_OUT_OF_STOCK);
        ...
        if ($num > 0) {
            ...
            if ($row) { //如果购物车已经有此物品，则更新
                ...
                if (C('use_storage') == 0 || $num <= $goods_storage) {
                    ...
                    SWTouch::err()->add(sprintf(L('shortage'), $num), ERR_OUT_OF_STOCK);
    ...
    function send_order_bonus($order_id) {
        ...
        if ($bonus_list) {
            ...
            if ($count > 0) {
                ...
                SWTouch::view()->assign('user_name', $user['user_name']);
                SWTouch::view()->assign('count', $count);
                SWTouch::view()->assign('money', $money);
                SWTouch::view()->assign('shop_name', C('shop_name'));
                SWTouch::view()->assign('send_date', local_date(C('date_format')));
                SWTouch::view()->assign('sent_date', local_date(C('date_format')));
                $content = SWTouch::view()->fetch('str:' . $tpl['template_content']);
    ...
    function add_package_to_cart($package_id, $num = 1) {
        ...
        if (C('use_storage') == '1' && model('Order')->judge_package_stock($package_id)) {
            SWTouch::err()->add(sprintf(L('shortage'), 1), ERR_OUT_OF_STOCK);
