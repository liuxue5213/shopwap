...
function insert_ads($arr) {
    ...
    $need_cache = SWTouch::view()->caching;
    SWTouch::view()->caching = false;

    SWTouch::view()->assign('ads', $ads);
    SWTouch::view()->assign('ads_array', $ads_array);
    $val = SWTouch::view()->fetch($position_style);

    SWTouch::view()->caching = $need_cache;
...
function insert_member_info() {
    ...
    if ($_SESSION['user_id'] > 0) {
        SWTouch::view()->assign('user_info', model('Users')->get_user_info());
    } else {
        if (!empty($_COOKIE['ECS']['username'])) {
            SWTouch::view()->assign('ecs_username', stripslashes($_COOKIE['ECS']['username']));
        }
        $captcha = intval(C('captcha'));
        if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0) {
            SWTouch::view()->assign('enabled_captcha', 1);
            SWTouch::view()->assign('rand', mt_rand());
        }
    }
    $output = SWTouch::view()->fetch('library/member_info.lbi');
...
function insert_comments($arr) {
    $need_cache = SWTouch::view()->caching;
    $need_compile = SWTouch::view()->force_compile;

    SWTouch::view()->caching = false;
    SWTouch::view()->force_compile = true;
    ...
    SWTouch::view()->assign('username', stripslashes($_SESSION['user_name']));
    SWTouch::view()->assign('email', $_SESSION['email']);
    SWTouch::view()->assign('comment_type', $arr['type']);
    SWTouch::view()->assign('id', $arr['id']);
    $comments = model('Comment')->get_comment_info($arr['id'], 0);
    SWTouch::view()->assign('comments_info', $comments);
    //全部评论
    $cmt = model('Comment')->assign_comment($arr['id'], $arr['type']);
    SWTouch::view()->assign('comment_list', $cmt['comments']);
    SWTouch::view()->assign('pager', $cmt['pager']);
    //好评
    $cmt_fav = model('Comment')->assign_comment($arr['id'], $arr['type'], '1');
    SWTouch::view()->assign('comment_fav', $cmt_fav['comments']);
    SWTouch::view()->assign('pager_fav', $cmt_fav['pager']);
    //中评
    $cmt_med = model('Comment')->assign_comment($arr['id'], $arr['type'], '2');
    SWTouch::view()->assign('comment_med', $cmt_med['comments']);
    SWTouch::view()->assign('pager_med', $cmt_med['pager']);
    //差评
    $cmt_bad = model('Comment')->assign_comment($arr['id'], $arr['type'], '3');
    SWTouch::view()->assign('comment_bad', $cmt_bad['comments']);
    SWTouch::view()->assign('pager_bad', $cmt_bad['pager']);
    $val = SWTouch::view()->fetch('library/comments_list.lbi');

    SWTouch::view()->caching = $need_cache;
    SWTouch::view()->force_compile = $need_compile;
...
function insert_bought_notes($arr) {
    ...
    SWTouch::view()->assign('notes', $bought_notes);
    SWTouch::view()->assign('pager', $pager);


    $val = SWTouch::view()->fetch('library/bought_notes.lbi');
...
function insert_vote() {
    $vote = get_vote();
    if (!empty($vote)) {
        SWTouch::view()->assign('vote_id', $vote['id']);
        SWTouch::view()->assign('vote', $vote['content']);
    }
    $val = SWTouch::view()->fetch('library/vote.lbi');
