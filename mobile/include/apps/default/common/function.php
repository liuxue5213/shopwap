...
function get_page_title($cat = 0, $str = '') {
    /* 初始化“页面标题”和“当前位置” */
    $page_title = C('shop_title') . ' - ' . 'Powered by SWTouch.cn';
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr = '', $url_format = '', $sch_array = '') {
    ...
    SWTouch::view()->assign('pager', $pager);
}
    ...
    if (!empty($upload['tmp_name'])) {
        ...
        if (!empty($ftype)) {
            ...
            if (!move_upload_file($upload['tmp_name'], $target)) {
                SWTouch::err()->add(L('upload_file_error'), 1);
            ...
            SWTouch::err()->add(L('upload_file_type'), 1);

            return false;
        }
    } else {
        SWTouch::err()->add(L('upload_file_error'));
        return false;
    }
}
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true) {
    ...
    $msg['type'] = $type;
    if (is_null(SWTouch::view()->get_template_vars('helps'))) {
        SWTouch::view()->assign('helps', model('Article')->get_shop_help()); // 网店帮助
    }

    SWTouch::view()->assign('title', L('tips_message'));
    SWTouch::view()->assign('auto_redirect', $auto_redirect);
    SWTouch::view()->assign('message', $msg);
    SWTouch::view()->display('message.dwt');
...
function assign_template($ctype = '', $catlist = array()) {
    SWTouch::view()->assign('image_width', C('image_width'));
    SWTouch::view()->assign('image_height', C('image_height'));
    SWTouch::view()->assign('points_name', C('integral_name'));
    SWTouch::view()->assign('qq', explode(',', C('qq')));
    SWTouch::view()->assign('ww', explode(',', C('ww')));
    SWTouch::view()->assign('ym', explode(',', C('ym')));
    SWTouch::view()->assign('msn', explode(',', C('msn')));
    SWTouch::view()->assign('skype', explode(',', C('skype')));
    SWTouch::view()->assign('stats_code', C('stats_code'));
    SWTouch::view()->assign('copyright', sprintf(L('copyright'), date('Y'), C('shop_name')));
    SWTouch::view()->assign('shop_name', C('shop_name'));
    SWTouch::view()->assign('service_email', C('service_email'));
    SWTouch::view()->assign('service_phone', C('service_phone'));
    SWTouch::view()->assign('shop_address', C('shop_address'));
    SWTouch::view()->assign('licensed', license_info());
    SWTouch::view()->assign('ecs_version', VERSION);
    SWTouch::view()->assign('icp_number', C('icp_number'));
    SWTouch::view()->assign('username', !empty($_SESSION['user_name']) ? $_SESSION['user_name'] : '');
    SWTouch::view()->assign('category_list', cat_list(0, 0, true, 2, false));
    SWTouch::view()->assign('catalog_list', cat_list(0, 0, false, 1, false));
    SWTouch::view()->assign('navigator_list', model('Common')->get_navigator($ctype, $catlist));  //自定义导航栏
    ...
    SWTouch::view()->assign('searchkeywords', $searchkeywords);
}
...
function set_affiliate($u = '') {
    ...
    if (!empty($_GET['u']) && $config['on'] == 1) {
        if (!empty($config['config']['expire'])) {
            ...
            setcookie('shopwapp_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * $config['config']['expire'] * $c);
        } else {
            setcookie('shopwapp_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24); // 过期时间为 1 天
        }
    }
}
...
function license_info() {
    if (C('licensed') > 0) {
        $license = '<a href="http://www.gope.cn/license.php?product=swtouch_free&url=' . urlencode(__URL__) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    } else {
        return '';
...
function edit_password($user_id, $old_password, $new_password = '', $code = '') {
    ...
    if (SWTouch::user()->edit_password($user_id, $old_password, $new_password, $code)) {
        return true;
    } else {
        SWTouch::err()->add(L('edit_password_failure'));
...
function check_userinfo($user_name, $email) {
    ...
    /* 检测用户名和邮件地址是否匹配 */
    $user_info = SWTouch::user()->check_pwd_info($user_name, $email);
    if (!empty($user_info)) {
        return $user_info;
    } else {
        return false;
...
function send_pwd_email($uid, $user_name, $email, $code) {
    ...
    SWTouch::view()->assign('user_name', $user_name);
    SWTouch::view()->assign('reset_email', $reset_email);
    SWTouch::view()->assign('shop_name', C('shop_name'));
    SWTouch::view()->assign('send_date', date('Y-m-d'));
    SWTouch::view()->assign('sent_date', date('Y-m-d'));
