...
class IndexController extends AdminController
{
    ...
    public function welcome()
    {
        ...
        // 检测是否授权
        $data = array('appid' => SWTOUCH_AUTH_KEY);
    ...
    public function login()
    {
        if (IS_POST) {
            ...
            $msg = Check::rule(array(
                ...
                array(
                    Check::same($captcha, $_SESSION['swtouch_verify']),
                    L('captcha_error')
                )
            ));
            // 提示信息
            ...
            if (! empty($userInfo)) {
                ...
                if (! empty($remember)) {
                    ...
                    setcookie('SWTOUCHCP[ADMIN_ID]', $userInfo['user_id'], $time);
                    setcookie('SWTOUCHCP[ADMIN_PWD]', md5(md5($userInfo['user_id'] . $userInfo['user_name']) . C('hash_code')), $time);
                }
                $result = array(
                    'err' => 0,
                    'msg' => 'login success'
    ...
    public function license()
    {
        if (IS_POST) {
            ...
            $data = array('license'=>$license, 'appid' => SWTOUCH_AUTH_KEY);
