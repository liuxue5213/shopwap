...
class WechatController extends AdminController
{
    ...
    public function article_edit()
    {
        if (IS_POST) {
            ...
            // 封面处理
            if ($_FILES['pic']['name']) {
                $result = $this->swtouchUpload('pic', 'article_pic');
                if ($result['error'] > 0) {
    ...
    public function picture()
    {
        if (IS_POST) {
            if ($_FILES['pic']['name']) {
                $result = $this->swtouchUpload('pic', 'article_pic', true);
                if ($result['error'] > 0) {
                    $this->message($result['message'], NULL, 'error');
                }
                $data['file'] = substr($result['message']['pic']['savepath'], 2) . $result['message']['pic']['savename'];
    ...
    public function voice()
    {
        if (IS_POST) {
            if ($_FILES['voice']['name']) {
                $result = $this->swtouchUpload('voice', 'voice');
                if ($result['error'] > 0) {
                    $this->message($result['message'], NULL, 'error');
                }
    ...
    public function video_upload()
    {
        if (IS_POST && ! empty($_FILES['file']['name'])) {
            ...
            $result = $this->swtouchUpload('file', 'video');
            if ($result['error'] > 0) {
                $data['errcode'] = 1;
