...
class TemplateController extends AdminController
{
    ...
    private function get_template_info($template_name, $template_style = '')
    {
        ...
        $css_path = ROOT_PATH . 'themes/' . $template_name . '/css/swtouch.css';
        if ($template_style != '') {
            $css_path = ROOT_PATH . 'themes/' . $template_name . "/css/swtouch_$template_style.css";
        }
    ...
    private function read_tpl_style($tpl_name, $flag = 1)
    {
        ...
        while ($file = readdir($tpl_style_dir)) {
            ...
            if ($file != '.' && $file != '..' && is_file($dir . $file) && $file != '.svn' && $file != 'index.htm') {
                if (preg_match("/^(swtouch|swtouch_)(.*)*/i", $file))                 // 取模板风格缩略图
                {
                    $start = strpos($file, '.');
