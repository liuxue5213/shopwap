...
class UpgradeController extends AdminController
{
    ...
    // md5验证地址
    private $_upgrademd5 = 'http://www.gope.cn/upgrademd5/';
    // 补丁地址
    private $_patchurl = 'http://download.shopwapp.com/upgrade/1.0/patch/';
    ...
    public function init()
    {
        ...
        foreach ($pathlist as $k => $v) {
            ...
            if (file_exists($configpath)) {
                ...
                // 版本文件地址
                $content = "<?php\ndefine('APPNAME', '".$config['APPNAME']."');\ndefine('VERSION', '".$config['VERSION']."');\ndefine('RELEASE', '".$release."');\ndefine('SWTOUCH_AUTH_KEY', '".SWTOUCH_AUTH_KEY."');";
    ...
    public function checkfile()
    {
        ...
        if (! empty($do)) {
            $this->ec_readdir('.');
            // 读取接口
            $swtouch_md5 = Http::doGet($this->_upgrademd5 . RELEASE . '_' . $this->patch_charset . ".php");
            $swtouch_md5_arr = json_decode($swtouch_md5, 1);
            $swtouch_md5_arr = empty($swtouch_md5_arr) ? array():$swtouch_md5_arr;
            // 计算数组差集
            $diff = array_diff($swtouch_md5_arr, $this->md5_arr);
            // 丢失文件列表
            $lostfile = array();
            foreach ($swtouch_md5_arr as $k => $v) {
            ...
            // 未知文件列表
            $unknowfile = array_diff(array_keys($this->md5_arr), array_keys($swtouch_md5_arr));
            // 赋值
