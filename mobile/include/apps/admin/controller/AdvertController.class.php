    ...
    public function ad_list(){
        ...
        foreach ($list as $key => $value) {
			...
			if($value['media_type'] == 0){
                if (strpos($value['ad_code'], 'http://') === false && strpos($value['ad_code'], 'https://') === false)
                {
                    $ad_list[$key]['ad_code'] = __URL__.'/'. $value['ad_code'];
                }else{
                    $ad_list[$key]['ad_code'] = $value['ad_code'];
                }
            }
            ...
            switch ($data['media_type']) {
                ...
                case '0':
                    ...
                    if ($_FILES['ad_img']['name']) {
                        /* ad_img广告图片 */
                        $result = $this->swtouchUpload('ad_img', 'ad_img');
                        if ($result['error'] > 0) {
