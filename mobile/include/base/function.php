...
function send_mail($name, $email, $subject, $content, $type = 0, $notification = false) {
    ...
    if (C('mail_service') == 0 && function_exists('mail')) {
        ...
        if (!$res) {
            SWTouch::err()->add(L('sendemail_false'));
     ...
     */ else {
        ...
        if (empty($params['host']) || empty($params['port'])) {
            ...
            SWTouch::err()->add(L('smtp_setting_error'));
            ...
            if (!function_exists('fsockopen')) {
                //如果fsockopen被禁用，直接返回
                SWTouch::err()->add(L('disabled_fsockopen'));
            ...
            if ($smtp->connect() && $smtp->send($send_params)) {
                ...
                if (empty($err_msg)) {
                    ...
                    if (strpos($err_msg, 'Failed to connect to server') !== false) {
                        SWTouch::err()->add(sprintf(L('smtp_connect_failure'), $params['host'] . ':' . $params['port']));
                    } else if (strpos($err_msg, 'AUTH command failed') !== false) {
                        SWTouch::err()->add(L('smtp_login_failure'));
                    } elseif (strpos($err_msg, 'bad sequence of commands') !== false) {
                        SWTouch::err()->add(L('smtp_refuse'));
                    } else {
                        SWTouch::err()->add($err_msg);
                    }
                }
...
function assign_query_info() {
    ...
    SWTouch::view()->assign('query_info', sprintf(L('query_info'), M()->queryCount, $query_time));

    /* 内存占用情况 */
    if (L('memory_info') && function_exists('memory_get_usage')) {
        SWTouch::view()->assign('memory_info', sprintf(L('memory_info'), memory_get_usage() / 1048576));
    }

    /* 是否启用了 gzip */
    $gzip_enabled = gzip_enabled() ? L('gzip_enabled') : L('gzip_disabled');
    SWTouch::view()->assign('gzip_enabled', $gzip_enabled);
}
...
function license_login($certi_added = '') {
    ...
    $certi['app_id'] = 'swtouch_free'; // 说明客户端来源
    ...
    $certi['certi_session'] = SWTouch::sess()->get_session_id(); // 网店SESSION标识
