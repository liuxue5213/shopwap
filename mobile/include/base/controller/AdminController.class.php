...
class AdminController extends BaseController {
    ...
    protected function isLogin() {
        if (empty($_SESSION[APP_NAME . '_USERINFO'])) {
            ...
            if (!empty($_COOKIE['SWTOUCHCP']['ADMIN_ID']) && !empty($_COOKIE['SWTOUCHCP']['ADMIN_PWD'])) {
                // 找到了cookie, 验证cookie信息
                $condition['user_id'] = intval($_COOKIE['SWTOUCHCP']['ADMIN_ID']);
                $userInfo = $this->model->table('admin_user')->field('user_id, user_name, password, email, last_login, ec_salt')->where($condition)->find();
                if (empty($userInfo)) {
                    // 没有找到这个记录
                    setcookie($_COOKIE['SWTOUCHCP']['ADMIN_ID'], '', 1);
                    setcookie($_COOKIE['SWTOUCHCP']['ADMIN_PWD'], '', 1);
                    return false;
                } else {
                    // 检查密码是否正确
                    if (md5(md5($userInfo['user_id'] . $userInfo['user_name']) . C('hash_code')) == $_COOKIE['SWTOUCHCP']['ADMIN_PWD']) {
    ...
    protected function clearLogin($url = '') {
        ...
        /* 清除cookie */
        setcookie('SWTOUCHCP[ADMIN_ID]', '', 1);
        setcookie('SWTOUCHCP[ADMIN_PWD]', '', 1);
    ...
    //$upload_dir上传的目录名
    protected function swtouchUpload($key = '', $upload_dir = 'images', $thumb = false, $width = 220, $height = 220) {
...
class_alias('AdminController', 'SWTouch');
