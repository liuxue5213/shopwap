...
function addToCart_quick(goodsId, parentId) {
 ...
 // 获取购物车数量统计
    var num_card = $('#num_card').html();
    num_card = parseInt(num_card)+parseInt(number);
    $.post('index.php?m=default&c=flow&a=add_to_cart', {
        goods: $.toJSON(goods)
    }, function(data) {
        addToCartResponse_quick(data,num_card);
    }, 'json');
 ...
 * 处理添加商品到购物车的反馈信息
 */
function addToCartResponse_quick(result,num_card) {
    if (result.error > 0) {
        // 如果需要缺货登记，跳转
...
//			}
    	...
    	$('#num_card').html(num_card);
    	$('#onclick_cart').click();
//        var cartInfo = document.getElementById('num_card');
//        var cart_url = 'index.php?m=default&c=flow&a=cart';
//        if (cartInfo) {
//            cartInfo.innerHTML = result.content;
//        }
