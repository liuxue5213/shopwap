...
if ($_REQUEST['act'] == 'list')
{
    ...
    $allow_set_points = $_CFG['integrate_code'] == 'shopwapp' ? 0 : 1;
...
if ($_REQUEST['act'] == 'install')
{
    ...
    if ($_GET['code'] == 'shopwapp')
    {
        $sql = "UPDATE " .$ecs->table('shop_config'). " SET value = 'shopwapp' WHERE code = 'integrate_code'";
        $db->query($sql);
...
if ($_REQUEST['act'] == 'check_user')
{
    ...
    $domain = empty($_GET['domain']) ? '@shopwapp' : trim($_GET['domain']);
    if ($size <2)
    {
        $size = 2;
    }
