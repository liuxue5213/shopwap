...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    assign_query_info();
    $smarty->assign('brand', array('sort_order'=>50, 'is_show'=>1, 'is_rec'=>1));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, is_rec, sort_order) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$is_rec', '$_POST[sort_order]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, is_rec, sort_order ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $param = "brand_name = '$_POST[brand_name]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', is_rec='$is_rec', sort_order='$_POST[sort_order]' ";
    if (!empty($img_name))
    {
        //有图片上传
...
//-- 切换是否推荐
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_rec')
{
    check_authz_json('brand_manage');
