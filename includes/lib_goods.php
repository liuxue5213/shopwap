...
/**
 * 获得指定分类同级的所有分类以及该分类下的子分类，优化数据库缓存
 * 技术支持：狗扑分享论坛(www.shopwapp.com)
 * @access  public
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
			...
			$sql = "SELECT b.brand_id, b.brand_logo,b.brand_name, COUNT(*) AS goods_num ".
					...
					"WHERE g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ") AND b.is_rec = 1 " .
